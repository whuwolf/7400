...
The \texttt{pareto} package includes the function \texttt{dpareto}, \texttt{ppareto} and \texttt{qpareto} to give the density, distribution function and quantile function for the Pareto distribution according to the arguments.
