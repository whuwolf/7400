...
@
\end{center}
As we can see, there is a tradeoff between bias and standard error for different methods. For example, \texttt{lowess} has a large bias but relatively small standard error. On the contrary, \texttt{smooth.spline} has a large standard error but small bias.

\item[(b)] Set the simulation sample size to be 1000 and let $\sigma$ range from 0.1 to 2, the graph of integrated mean square error is given below.
\begin{center}
<<echo = FALSE, fig = TRUE>>=
# (b)
sigma <- seq(0.1, 2, 0.1)
...
@
\end{center}
As sigma increases, the integrated mean square error will also increase. For this particular problem, \texttt{gam} has the best performance and \texttt{smooth.spline} performs the worst in terms of mean square error.
...
\section*{Problem 2}
\begin{enumerate}
\item[(a)] Compare \texttt{dpareto} and \texttt{p.dpareto}.
\begin{center}
<<echo = FALSE, fig = TRUE>>=
# Problem 2 
library(pareto)
...
matplot(n, dpareto.time, type = "l", main = "dpareto Timings", xlab = "log10(n)", 
        ylab = "time")
legend("topleft",legend=c("P=1","P=2"),lty=c(1,2),col=c("black","red"))
@
\end{center}

\item[(b)] Compare \texttt{ppareto} and \texttt{p.ppareto}.
\begin{center}
<<echo = FALSE, fig = TRUE>>=
#ppareto
...
matplot(n, ppareto.time, type = "l", main = "ppareto Timings", xlab = "log10(n)", 
        ylab = "time")
legend("topleft",legend=c("P=1","P=2"),lty=c(1,2),col=c("black","red"))
@
\end{center}

\item[(c)] Compare \texttt{qpareto} and \texttt{p.qpareto}.
\begin{center}
<<echo = FALSE, fig = TRUE>>=
#qpareto
n <- seq(1, 6, 0.1)
...
matplot(n, qpareto.time, type = "l", main = "qpareto Timings", xlab = "log10(n)", 
        ylab = "time")
legend("topleft",legend=c("P=1","P=2"),lty=c(1,2),col=c("black","red"))
@
\end{center}
